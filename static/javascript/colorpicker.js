(function () {
	...
	Colorpicker.prototype = {
		init(opt) {
			let { el, initColor = "rgb(0,0,0)", allMode = ['hex', 'rgb'], color = '',input_name = '' } = opt;
			...
			this.current_mode = 'rgb'; // input框当前的模式
...
//				div = document.createElement("div");
			...
			var body = document.getElementById("settingPage"),
				div = document.createElement("div");
            ...
            // 显示偏移位置
			this.pancelLeft = left + this.elem_colorPalette.clientWidth - 127;
			this.pancelTop = top + this.bindElem.offsetHeight -40;
			util.css(div, {
				"position": "absolute",
				"z-index": 99999999,
				"display": 'none',
				"left": left -127 + "px",
				"top": top - 40 + this.bindElem.offsetHeight + "px"
			});
				...
				if (e.target.tagName.toLocaleLowerCase() == "p") {
					...
					switch (_this.current_mode) {
						...
						case 'rgb':
							...
							rgb_string = "\"" + rgba.r + "," + rgba.g + "," + rgba.b + "\"";
//							$("#process_background_color").val(rgb_string);
							$("#" + opt.input_name).val(rgb_string);
			...
			this.elem_colorPalette.addEventListener("dblclick", function (e) {
                clearTimeout(timer);
                console.log("2");
				if (e.target.tagName.toLocaleLowerCase() == "p") {
					let colorStr = e.target.style.background;
...
//							$("#process_background_color").val(rgb_string);
		...
		render: function () {
			var tpl =
				...
				<div style="position: inherit;z-index: 100;display: flex;box-shadow: rgba(0, 0, 0, 0.3) 0px 0px 2px, rgba(0, 0, 0, 0.3) 0px 4px 8px;">
					<div style='width:180px; padding:5px;background: #f9f9f9;display: flex;flex-flow: row wrap;align-content: space-around;justify-content: space-around;' class='color-palette'>
						${this.getPaletteColorsItem()}
					</div>
					<div class="colorpicker-pancel" style="background: rgb(255, 255, 255);box-sizing: initial; width: 225px; font-family: Menlo; display:none;">
					...
					current_mode_html += `
							<div style="padding-left: 6px; width: 100%;">
								<div style="position: relative;">
									<input class="colorpicker-hexInput" readonly="readonly" value="${hex}" spellcheck="false" style="font-size: 11px; color: rgb(51, 51, 51); width: 100%; border-radius: 2px; border: none; box-shadow: rgb(218, 218, 218) 0px 0px 0px 1px inset; height: 21px; text-align: center;">
				...
				case 'rgb':
					for (var i = 0; i < 3; i++) {
						current_mode_html +=
							`<div style="padding-left: 6px; width: 100%;">
								<div style="position: relative;">
									<input class="colorpicker-hexInput" readonly="readonly" value="${this.rgba['rgb'[i]]}" spellcheck="false" style="font-size: 11px; color: rgb(51, 51, 51); width: 100%; border-radius: 2px; border: none; box-shadow: rgb(218, 218, 218) 0px 0px 0px 1px inset; height: 21px; text-align: center;">
		...
		getPaletteColorsItem: function () {
			let str = '';
			// 初始化面板50个颜色
			let palette = ["rgb(128,128,128)","rgb(13,13,13)","rgb(22,22,22)","rgb(34,43,53)","rgb(31,78,120)","rgb(131,60,12)","rgb(82,82,82)","rgb(128,96,0)","rgb(32,55,100)","rgb(55,86,35)",,"rgb(166,166,166)","rgb(38,38,38)","rgb(58,56,56)","rgb(51,63,79)","rgb(47,117,181)","rgb(198,89,17)","rgb(123,123,123)","rgb(191,143,0)","rgb(48,84,150)","rgb(84,130,53)",,"rgb(191,191,191)","rgb(64,64,64)","rgb(117,113,113)","rgb(132,151,176)","rgb(155,194,230)","rgb(244,176,132)","rgb(201,201,201)","rgb(255,217,102)","rgb(142,169,219)","rgb(169,208,142)",,"rgb(217,217,217)","rgb(89,89,89)","rgb(174,170,170)","rgb(172,185,202)","rgb(189,215,238)","rgb(248,203,173)","rgb(219,219,219)","rgb(255,230,153)","rgb(180,198,231)","rgb(198,224,180)",,"rgb(242,242,242)","rgb(128,128,128)","rgb(208,206,206)","rgb(214,220,228)","rgb(221,235,247)","rgb(252,228,214)","rgb(237,237,237)","rgb(255,242,204)","rgb(217,225,242)","rgb(226,239,218)"]
			palette.forEach(item => str += `<p style='width:15px;height:15px;background:${item};margin:0 1px;border: solid 1px #d0d0d0;'></p>`)
			return str;
		},
		setPosition(x, y) {
