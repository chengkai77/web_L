(function($) {
        ...
        ySelect.prototype = {
            create: function() {
                ...
                if (settings.type && settings.type == "input"){
                    this.$select.before('<div class="fs-label-wrap"><input class="property-input fs-label" id="'+settings.id+'" autocomplete=\"off\" placeholder="'+ this.settings.placeholder + '"/><span class="fs-arrow"></span></div>');
                }else{
                    this.$select.before('<div class="fs-label-wrap"><div class="fs-label" id="'+settings.id+'">' + this.settings.placeholder + '</div><span class="fs-arrow"></span></div>');
                }
            ...
            reloadDropdownLabel: function() {
                ...
                if (settings.type && settings.type == "input"){
                    this.$wrap.find('.fs-label').val(labelText);
                    let div_id = this.$wrap.find('.fs-label')[0].id.replace("");
                    $("div_id")
                }else{
        ...
        if (0 < $wrap.length) {
            if ($el.hasClass('fs-label')||$el.hasClass('fs-arrow')) {
                ...
                if (is_hidden) {
                    ...
                    if (document.getElementById("pivot_table_filters_values")){
                        let focusInputId = $wrap.find('.fs-label-wrap')[0].firstChild.id;
                        let inputsList = ['pivot_table_filters_values','pivot_table_columns_values','pivot_table_rows_values'];
                        let inputIndex = inputsList.indexOf(focusInputId);
                        delete inputsList[inputIndex];
                        /*inputsList = inputsList.splice(inputsList.indexOf(focusInputId),1);*/
                        let inputsChosedList = [];
                        inputsList.forEach(function (value) {
                            let cur_list = $("#"+value).val().split(", ");
                            cur_list.forEach(function (val) {
                                if (inputsChosedList.indexOf(val) == -1){
                                    inputsChosedList.push(val);
                                }
                            })
                        })
                        let optionDivs = $wrap.find('.fs-dropdown').find("div[class='fs-option']");
                        for (var i=0;i<optionDivs.length;i++){
                            let optionText = optionDivs[i].innerText;
                            if (inputsChosedList.indexOf(optionText) != -1){
    ...
    $.fn.ySelectedGetValues=function ()
      {
        if (typeof(settings) != "undefined"){
            var result = $('#'+settings.id)[0].innerText;
        }else{
            try{
                var result = $('#'+this[0].id+"_values").val() || $('#'+this[0].id+"_values")[0].innerText;
            }catch(e){
                var result = $('#'+this[0].id+"_values")[0].innerText;
            }
        }
        return result;
      }
